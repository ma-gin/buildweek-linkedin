...
const GetAPost = () => {
  ...
  <span className="sr-only">Loading...</span>
      ...
      {posts && posts.filter((post,idx)=>idx > posts.length - 20 && idx < posts.length).map((post) =>  <SinglePost username={post.user.name + ' ' + post.user.surname} text={post.text} key={post._id} unique={post._id}/>
      )}
    </>
  );
};
